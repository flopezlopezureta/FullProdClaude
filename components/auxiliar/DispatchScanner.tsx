...
const ScannerView: React.FC<ScannerViewProps> = ({ initialDriver, allDrivers, onBack }) => {
    ...
    const [scanResult, setScanResult] = useState<{ type: 'success' | 'error' | 'info'; message: string; package?: any } | null>(null);
        ...
        const performDispatch = async (force = false) => {
            try {
              if (isFalabellaDirect) {
                  ...
                  // creates it already assigned to currentDriverId, in one step. That requires a
                  // live lookup against Falabella's API for the recipient's real data, which is
                  // the actual source of the delay here — surface it so the wait doesn't read as
              ...
              const successMessage = `Paquete ${codeToUse} asignado a ${currentDriver.name}`;
